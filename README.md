...
>
> **v0.8.1 targets Skyrim SE/AE 1.6.640 with RaceMenu 0.4.19.14.** The DLL deliberately refuses to load on another Skyrim runtime so compatibility reports remain isolated.
...
v0.8.1 now queries the ABI-stable SKEE base interface first and selects a backend from the actual interface versions:
...
The v0.8.1 test FOMOD includes the provider set inherited from v0.8.0:
...
3. Install `BodyHairSliders-v0.8.1-SE-1.6.640-RM-0.4.19.14-TEST-FOMOD.zip`.
...
8. Test Hair sliders, `None / Shaved`, color changes, and reopening RaceMenu.
...
```text
dist/BodyHairSliders-v0.8.1-SE-1.6.640-RM-0.4.19.14-TEST-FOMOD.zip
```
